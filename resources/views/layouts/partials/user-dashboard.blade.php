...
<div class="w-full relative grid grid-cols-1 lg:grid-cols-2 gap-6">
  <div class="w-full block">
    ...
    <div class="profile-header w-full p-[20px] rounded-md relative mb-4 bg-[#ADDF98]">
      <div class="flex gap-4 lg:gap-6 items-center flex-wrap w-full mb-4">
        ...
        <div class="flex flex-col gap-1 items-start">
          <h3 class="text-[24px] text-[#0E0E0E]">{{ Auth::user()->name ?? 'User Name' }}</h3>
          ...
          <a href="/profile" class="h-[40px] rounded-md bg-[#3C8DBC] hover:bg-[#85C16D] transition-all ease-in-out duration-300 text-white w-auto text-[14px] leading-tight py-[8px] px-[20px] mt-2 flex items-center justify-center">Update Profile</a>
        </div>
      </div>
    </div>
    ...
    <div class="dast-std-status w-full p-[20px] rounded-md relative mb-4 bg-[#ADDF98]">
      <h2 class="text-[20px] md:text-[22px] lg:text-[24px] text-black font-semibold mb-2">My Training Status</h2>
      <div class="relative w-full grid grid-cols-2 md:grid-cols-3 gap-4">
        <div class="flex flex-col w-full rounded-md min-h-[120px] h-full text-black items-center justify-center text-center p-[10px] shadow-sm bg-[#F8FCE9]">
          <span class="text-[14px] block text-[#686868]">Total Enrollments</span>
          <span class="text-[20px] font-semibold">{{ \App\Models\TrainingParticipant::where('email', Auth::user()->email)->count() }}</span>
        </div>
        ...
        <div class="flex flex-col w-full rounded-md min-h-[120px] h-full text-black items-center justify-center text-center p-[10px] shadow-sm bg-[#ECFAFC]">
          <span class="text-[14px] block text-[#686868]">Completed</span>
          <span class="text-[20px] font-semibold">{{ \App\Models\TrainingParticipant::where('email', Auth::user()->email)->where('is_training_completed', true)->count() }}</span>
        </div>

        <div class="flex flex-col w-full rounded-md min-h-[120px] h-full text-black items-center justify-center text-center p-[10px] shadow-sm bg-[#F8FCE9]">
          <span class="text-[14px] block text-[#686868]">In Progress</span>
          <span class="text-[20px] font-semibold">{{ \App\Models\TrainingParticipant::where('email', Auth::user()->email)->where('status', true)->where('is_training_completed', false)->count() }}</span>
        </div>

        <div class="flex flex-col w-full rounded-md min-h-[120px] h-full text-black items-center justify-center text-center p-[10px] shadow-sm bg-[#ECFAFC]">
          <span class="text-[14px] block text-[#686868]">Pending</span>
          <span class="text-[20px] font-semibold">{{ \App\Models\TrainingParticipant::where('email', Auth::user()->email)->where('status', false)->count() }}</span>
        </div>

        <div class="flex flex-col w-full rounded-md min-h-[120px] h-full text-black items-center justify-center text-center p-[10px] shadow-sm bg-[#F8FCE9]">
          <span class="text-[14px] block text-[#686868]">Certificates</span>
          <span class="text-[20px] font-semibold">{{ \App\Models\TrainingParticipant::where('email', Auth::user()->email)->whereNotNull('certificate_url')->count() }}</span>
        </div>

        <div class="flex flex-col w-full rounded-md min-h-[120px] h-full text-black items-center justify-center text-center p-[10px] shadow-sm bg-[#ECFAFC]">
          <span class="text-[14px] block text-[#686868]">Available Courses</span>
          <span class="text-[20px] font-semibold">{{ \App\Models\Training::where('status', 1)->count() }}</span>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="w-full block">
    <div class="dast-std-status w-full p-[20px] rounded-md relative mb-4 bg-[#ADDF98]">
      <h2 class="text-[20px] md:text-[22px] lg:text-[24px] text-[#000B8C] font-semibold mb-4">Recent Activity</h2>
...
<div class="std-progress-sec w-full relative my-10">
  <div class="w-full max-w-[640px] mx-auto relative">
    <h2 class="text-[22px] font-semibold text-center mb-6 text-[#0E0E0E]">Learning Progress</h2>
    <div class="w-full relative grid grid-cols-1 lg:grid-cols-2 gap-6">
      @php
