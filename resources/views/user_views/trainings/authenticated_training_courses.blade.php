            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[30px] 2xl:gap-[70px]">
              ...
              <div class="trc-card-item relative overflow-hidden w-full rounded-[8px] bg-white border-b-[4px] border-[#85C16D] py-[30px] px-[15px]">
                ...
                <div class="relative z-10 flex flex-col h-full items-center justify-center text-center">
                  ...
                  <h2 class="text-[20px] text-[#474D49] py-1 font-bold capitalize">Total Registration <br> 
                    @if(Auth::check())
                      {{ \App\Models\TrainingParticipant::where('email', Auth::user()->email)->count() }}
                    @else
                      02
                    @endif
                  </h2>
              ...
              <div class="skd-tab-content relative w-full block">
                    ...
                    <div class="relative w-full block mb-6">
                      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[30px] w-full">
                        @foreach($batch as $singleBatch)
                          <div class="trc-course-item relative overflow-hidden w-full rounded-[8px] bg-white border border-[#DBF5D0] pt-[10px] px-[10px] pb-[70px]">
                            <div class="relative z-10 flex flex-col h-full items-start justify-start text-center">
                              <div class="trc-course-item-img h-[175px] block w-full mb-4">
                                <img src="{{ asset('storage/' . ($singleBatch->training->course_thumbnail ?? 'assets/images/dash-course-img.png')) }}" alt="Training Image" class="w-full h-full object-cover">
                              </div>
                                  ...
                                  <div class="flex flex-col gap-2 flex-wrap mb-2 w-full">
                                    <div class="relative pl-[24px] inline-block mr-4">
                                      ...
                                      <p class="text-[13px] leading-3 flex min-h-[20px] items-center text-left">
                                        Durations: {{ \Carbon\Carbon::parse($singleBatch->start_date)->format('d-M-Y') }} to {{ \Carbon\Carbon::parse($singleBatch->end_date)->format('d-M-Y') }}
                                      </p>
                                    </div>
                                    <div class="relative pl-[24px] inline-block mr-4">
                                      <span class="absolute left-0 top-[2px] h-[16px] w-[16px] bg-[#00A65A] rounded-sm block"></span>
                                      <p class="text-[13px] leading-3 flex min-h-[20px] items-center text-left">
                                        Total Hours: {{ $singleBatch->class_duration }}
                                      </p>
                                    </div>
                                  </div>
                                  <h2 class="text-[15px] md:text-[16px] leading-2 mt-1 font-medium">{{ $singleBatch->training->name }}</h2>
                                  <p class="text-[14px] leading-5 py-1 text-left w-full">Batch: {{ $singleBatch->name }}</p>
                                  <p class="text-[14px] leading-5 py-1 text-left w-full">
                                    Status: {{ $singleBatch->batch_status == 1 ? 'ONGOING' : ($singleBatch->batch_status == 0 ? 'COMPLETE' : 'PENDING') }}
                                  </p>
                                  <p class="text-[14px] leading-5 py-1 text-left w-full">Training Medium: {{ $singleBatch->venue }}</p>
                                  <p class="text-[14px] leading-5 py-1 text-left w-full text-[#892626]">Registration Ends: {{ \Carbon\Carbon::parse($singleBatch->enrollment_deadline)->format('d-M-Y') }}</p>
                                </div>
                              </div>
                            </div>
                            @if(Auth::check())
                              <a class="h-[40px] absolute bottom-[10px] right-[10px] rounded-lg bg-[#3C8DBC] hover:bg-[#85C16D] transition-all ease-in-out duration-300 text-white flex items-center justify-center gap-2 w-full max-w-[170px] md:w-[140px] text-[16px] py-2 px-4" 
                                 href="{{ route('userCourse.view', $singleBatch->id) }}">
                            ...
                            @else
                              <a class="h-[40px] absolute bottom-[10px] right-[10px] rounded-lg bg-[#3C8DBC] hover:bg-[#85C16D] transition-all ease-in-out duration-300 text-white flex items-center justify-center gap-2 w-full max-w-[170px] md:w-[140px] text-[16px] py-2 px-4" 
                                 href="{{ route('course.view', $singleBatch->id) }}">
                                <span class="text-[16px]">View Course</span>
                                <span class="inline-block">
                                  <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 21 21" fill="none">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M10.5 19.7498C15.608 19.7498 19.75 15.6088 19.75 10.4998C19.75 5.39182 15.608 1.24982 10.5 1.24982C5.392 1.24982 1.25 5.39182 1.25 10.4998C1.25 15.6088 5.392 19.7498 10.5 19.7498Z" stroke="#F9FAFC" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                ...
                @if(Auth::check())
                  <div x-show="tab === 'tabSkdMyCourse'" class="tab-content-block">
                    <div class="tab-main-content relative w-full block">
                      <div class="relative w-full block mb-6">
                        @if(isset($enrolledBatches) && $enrolledBatches->count() > 0)
                          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[30px] w-full">
