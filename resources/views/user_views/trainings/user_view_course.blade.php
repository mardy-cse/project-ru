...
<div>
    ...
    <main id="mainContent" class="skd-course-view flex-1 overflow-x-hidden overflow-y-auto bg-[#F9FAFC]">
        <div class="container mx-auto px-6 py-8">
            ...
            @if(session('error'))
                <div class="alert alert-error mb-6 p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg">
            ...
            @if(session('info'))
                <div class="alert alert-info mb-6 p-4 bg-blue-100 border border-blue-400 text-blue-700 rounded-lg">
            ...
            <div class="flex flex-wrap flex-col lg:flex-row w-full gap-[20px]">
                
                {{-- Left Column --}}
                <div class="skd-card-col flex flex-col gap-[20px] relative w-full md:min-w-[340px] lg:max-w-[400px] order-1">
                    ...
                    <div class="trc-course-item relative w-full rounded-[8px] bg-white border border-[#85C16D] py-[10px] px-[10px]">
                        <div class="relative z-10 flex flex-col h-full items-start justify-start text-center pb-[20px]">
                            <div class="trc-course-item-img h-[200px] block w-full mb-4">
                                <img src="{{ asset('storage/' . ($batch->speaker->profile_image ?? 'assets/images/dash-course-img.png')) }}" alt="Trainer Image" class="w-full h-full object-cover rounded">
                            </div>
                            <div class="trc-course-item-desc w-full block relative">
                                <div class="skd-course-desc flex items-start justify-center flex-col w-full text-left text-[#000000]">
                ...
                {{-- Right Column --}}
                <div class="skd-card-col flex flex-col gap-[20px] relative w-full flex-grow order-2">
                    
                    {{-- Course Content Details Card --}}
                    <div class="trc-course-item relative w-full rounded-[8px] bg-white border border-[#85C16D] py-[10px] px-[10px]">
                        <div class="w-full mb-4">
                            <h3 class="text-[#85C16D] text-[18px] font-normal mb-1">Overview</h3>
                            <div class="text-[14px] leading-5 pb-2 text-left w-full">
                                {!! $batch->training->training_overview ?? '<p>No overview available</p>' !!}
                            </div>
                        </div>
                        ...
                        <div class="w-full mb-4">
                            ...
                            <div class="text-[14px] leading-5 pb-2 text-left w-full">
                                {!! $batch->training->training_objective ?? '<p>No objective available</p>' !!}
                            </div>
                        </div>
                        ...
                        <div class="w-full mb-4">
                            ...
                            <div class="text-[14px] leading-5 pb-2 text-left w-full">
                                {!! $batch->training->training_objective ?? '<p>No learning outcomes available</p>' !!}
                            </div>
                        </div>
                    </div>
                                    ...
                                    <table class="w-full text-[12px] xl:text-[13px] text-center text-[#545D69] min-w-[580px]">
                                        <thead>
                                            <tr class="bg-[#F0F0F0] font-normal align-baseline text-[#000000]">
                                                <th class="px-3 py-2 2xl:py-3 font-normal align-middle w-[80px]">Day</th>
                                                <th class="px-3 py-2 2xl:py-3 font-normal align-middle">Time</th>
                                                <th class="px-3 py-2 2xl:py-3 font-normal align-middle">Location</th>
                                                <th class="px-3 py-2 2xl:py-3 font-normal align-middle" width="100">Seats</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr class="bg-[#FCFCFC]">
                                                <td class="text-center py-2">{{ $batch->session_day ?? 'N/A' }}</td>
                                                <td class="text-center py-2">{{ ($batch->start_time ?? 'N/A') . ' to ' . ($batch->end_time ?? 'N/A') }}</td>
                                                <td class="text-center py-2">{{ $batch->venue ?? 'N/A' }}</td>
                                                <td class="text-center py-2">{{ $batch->seat_capacity ?? 'N/A' }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                                ...
                                @else
                                    ...
                                    @php
                                        $enrolledCount = \App\Models\TrainingParticipant::where('batch_id', $batch->id)
                                            ->where('status', true)
                                            ->count();
                                        $availableSeats = $batch->seat_capacity - $enrolledCount;
                                    @endphp
                                    
                                    <div class="mt-3 text-sm">
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                                            @if($availableSeats > 10) bg-green-100 text-green-800
                                            @elseif($availableSeats > 5) bg-yellow-100 text-yellow-800
                                            @elseif($availableSeats > 0) bg-orange-100 text-orange-800
                                            @else bg-red-100 text-red-800
                                            @endif">
                                            {{ $availableSeats }} of {{ $batch->seat_capacity }} seats available
                                        </span>
                                    </div>
                                @endif
                            </div>
                                    ...
                                    @else
                                        @php
                                            $enrolledCount = \App\Models\TrainingParticipant::where('batch_id', $batch->id)
                                                ...
                                                ->count();
                                            $availableSeats = $batch->seat_capacity - $enrolledCount;
                                            $enrollmentDeadlinePassed = \Carbon\Carbon::now()->gt(\Carbon\Carbon::parse($batch->enrollment_deadline));
                                        @endphp
                                        
                                        @if($enrollmentDeadlinePassed)
                                            <button type="button" disabled 
                                                class="flex items-center justify-center py-[5px] px-[15px] bg-gray-400 text-[16px] leading-tight text-white font-normal rounded-md mt-4 h-[40px] cursor-not-allowed">
                                                <i class="fas fa-clock mr-2"></i>
                                                Enrollment Closed
                                            </button>
                                        ...
                                        @else
                                            {{-- Enroll Now Button --}}
                                            <form action="{{ route('training.enroll', $batch->id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit" 
                                ...
                                <div class="flex relative items-center justify-center p-[10px] min-w-[50px] h-[50px] w-[50px] rounded-full bg-[#F3FFEE]">
                                    ...
                                    <svg class="w-[24px]" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                        <g clip-path="url(#clip0_19_1233)">
                                            ...
                                            <path d="M10.7732 15.4128C10.7732 15.154 10.5633 14.9441 10.3044 14.9441H3.83887C3.57998 14.9441 3.37012 15.154 3.37012 15.4128C3.37012 15.6717 3.57998 15.8816 3.83887 15.8816H10.3045C10.5633 15.8816 10.7732 15.6717 10.7732 15.4128Z" fill="url(#paint6_linear_19_1233)"></path>
                                            <path d="M3.83887 16.9089C3.57998 16.9089 3.37012 17.1188 3.37012 17.3777C3.37012 17.6365 3.57998 17.8464 3.83887 17.8464H6.88316C7.14205 17.8464 7.35191 17.6365 7.35191 17.3777C7.35191 17.1188 7.14205 16.9089 6.88316 16.9089H3.83887Z" fill="url(#paint7_linear_19_1233)"></path>
                                            <path d="M0.0797892 19.7588C0.221492 19.9715 0.517039 20.0291 0.729477 19.8882C0.942477 19.7469 0.999899 19.4502 0.858852 19.2381C0.717524 19.0255 0.421133 18.9675 0.209164 19.1087C-0.00482015 19.2512 -0.0598514 19.5454 0.0797892 19.7588Z" fill="url(#paint8_linear_19_1233)"></path>
                                        </g>
                                        <defs>
                                            <linearGradient id="paint0_linear_19_1233" x1="-0.361895" y1="13.6216" x2="23.9999" y2="13.6921" gradientUnits="userSpaceOnUse"><stop stop-color="#CDFCBA"></stop><stop offset="1" stop-color="#ADDF98"></stop></linearGradient>
                                            <linearGradient id="paint1_linear_19_1233" x1="3.02342" y1="6.09268" x2="9.26978" y2="6.10889" gradientUnits="userSpaceOnUse"><stop stop-color="#CDFCBA"></stop><stop offset="1" stop-color="#ADDF98"></stop></linearGradient>
                                            <linearGradient id="paint2_linear_19_1233" x1="10.9447" y1="3.66417" x2="15.387" y2="3.72417" gradientUnits="userSpaceOnUse"><stop stop-color="#CDFCBA"></stop><stop offset="1" stop-color="#ADDF98"></stop></linearGradient>
                                            <linearGradient id="paint3_linear_19_1233" x1="10.9447" y1="5.72448" x2="15.387" y2="5.78448" gradientUnits="userSpaceOnUse"><stop stop-color="#CDFCBA"></stop><stop offset="1" stop-color="#ADDF98"></stop></linearGradient>
                                            <linearGradient id="paint4_linear_19_1233" x1="10.9447" y1="7.78454" x2="15.387" y2="7.84453" gradientUnits="userSpaceOnUse"><stop stop-color="#CDFCBA"></stop><stop offset="1" stop-color="#ADDF98"></stop></linearGradient>
                                            <linearGradient id="paint5_linear_19_1233" x1="3.02137" y1="12.1676" x2="15.3865" y2="12.302" gradientUnits="userSpaceOnUse"><stop stop-color="#CDFCBA"></stop><stop offset="1" stop-color="#ADDF98"></stop></linearGradient>
                                            <linearGradient id="paint6_linear_19_1233" x1="3.25848" y1="15.4762" x2="10.7695" y2="15.6478" gradientUnits="userSpaceOnUse"><stop stop-color="#CDFCBA"></stop><stop offset="1" stop-color="#ADDF98"></stop></linearGradient>
                                            <linearGradient id="paint7_linear_19_1233" x1="3.31007" y1="17.441" x2="7.35142" y2="17.4907" gradientUnits="userSpaceOnUse"><stop stop-color="#CDFCBA"></stop><stop offset="1" stop-color="#ADDF98"></stop></linearGradient>
                                            <linearGradient id="paint8_linear_19_1233" x1="-0.0111415" y1="19.5615" x2="0.936096" y2="19.5642" gradientUnits="userSpaceOnUse"><stop stop-color="#CDFCBA"></stop><stop offset="1" stop-color="#ADDF98"></stop></linearGradient>
                                            <clipPath id="clip0_19_1233"><rect width="24" height="24" fill="white"></rect></clipPath>
                                        </defs>
                                    </svg>
                                </div>
                            </div>
